...
from gwe.nvidia.minx import XData
from gwe.nvidia.nvtarget import NV_CTRL_TARGET_TYPE_GPU, GPU, Screen, Target
...
class _NVCtrlQueryExtensionRequest:
    ...
    def __init__(self, opcode: int) -> None:
        self.encoding = minx.encode(
            ...
            XData('CARD8', 1, _X_nvCtrlQueryExtension),
            XData('CARD16', 1, 1))
...
class _NVCtrlQueryExtensionReply:
    ...
    def __init__(self, encoding: bytes) -> None:
        xreply, ad = minx.decode(encoding,
                                 ...
                                 XData('CARD16', 1, 'major'),
                                 XData('CARD16', 1, 'minor'),
                                 XData('CARD32', 1, 'padl4'),
                                 XData('CARD32', 1, 'padl5'),
                                 XData('CARD32', 1, 'padl6'),
                                 XData('CARD32', 1, 'padl7'),
                                 XData('CARD32', 1, 'padl8'))
...
class NVCtrlQueryAttributeRequest:
    ...
    def __init__(self, opcode: int, target_id: int, target_type: int, display_mask: int, attr: int):
        self.encoding = minx.encode(
            XData('CARD8', 1, opcode),
            XData('CARD8', 1, _X_nvCtrlQueryAttribute),
            XData('CARD16', 1, 4),
            XData('CARD16', 1, target_id),
            XData('CARD16', 1, target_type),
            XData('CARD32', 1, display_mask),
            XData('CARD32', 1, attr))
...
class _NVCtrlQueryTargetCountRequest:
    ...
    def __init__(self, opcode, target) -> None:
        self.encoding = minx.encode(
            XData('CARD8', 1, opcode),
            XData('CARD8', 1, _X_nvCtrlQueryTargetCount),
            XData('CARD16', 1, 2),
            XData('CARD32', 1, target))
...
class _NVCtrlQueryTargetCountReply:
    ...
    def __init__(self, encoding: bytes) -> None:
        x_reply, ad = minx.decode(encoding,
                                  ...
                                  XData('CARD32', 1, 'padl8'))
        self.type = x_reply['type']
        self.padb1 = x_reply['padb1']
        self.sequence_number = x_reply['sequence_number']
        self.length = x_reply['length']
        self.count = x_reply['count']
        self.padl4 = x_reply['padl4']
        self.padl5 = x_reply['padl5']
        self.padl6 = x_reply['padl6']
        self.padl7 = x_reply['padl7']
        self.padl8 = x_reply['padl8']
...
class _NVCtrlQueryBinaryDataRequest:
    ...
    def __init__(self, opcode, target_id, target_type, display_mask, attr):
        self.encoding = minx.encode(
            XData('CARD8', 1, opcode),
            XData('CARD8', 1, _X_nvCtrlQueryBinaryData),
            XData('CARD16', 1, 4),
            XData('CARD16', 1, target_id),
            XData('CARD16', 1, target_type),
            XData('CARD32', 1, display_mask),
...
class _NVCtrlQueryStringAttributeRequest:
    ...
    def __init__(self, opcode, target_id, target_type, display_mask, attr) -> None:
        self.encoding = minx.encode(
            XData('CARD8', 1, opcode),
            XData('CARD8', 1, _X_nvCtrlQueryStringAttribute),
            XData('CARD16', 1, 4),
            XData('CARD16', 1, target_id),
            XData('CARD16', 1, target_type),
            XData('CARD32', 1, display_mask),
...
class NVCtrlQueryStringAttributeReply:
    ...
    def __init__(self, encoding: bytes) -> None:
        xreply, ad = minx.decode(encoding,
                                 ...
                                 XData('CARD32', 1, 'pad6'),
                                 XData('CARD32', 1, 'pad7'))
        self.type = xreply['type']
        self.pad0 = xreply['pad0']
        self.sequence_number = xreply['sequence_number']
        self.length = xreply['length']
        self.flags = xreply['flags']
        self.n = xreply['n']
        self.pad4 = xreply['pad4']
        self.pad5 = xreply['pad5']
        self.pad6 = xreply['pad6']
        self.pad7 = xreply['pad7']

        rs, ad = minx.decode(ad, XData('STRING8', self.n, 'string'))
        self.string = str(rs['string'])
        if sys.version_info > (3, 0) and isinstance(self.string, bytes):
...
class _NVCtrlSetStringAttributeRequest:
    def __init__(self, opcode, screen, display_mask, attr, data):
        ...
        self.encoding = minx.encode(
            XData('CARD8', 1, opcode),
            XData('CARD8', 1, _X_nvCtrlSetStringAttribute),
            XData('CARD16', 1, 5 + (((dlen + 3) & ~3) >> 2)),
            XData('CARD32', 1, screen),
            XData('CARD32', 1, display_mask),
            XData('CARD32', 1, attr),
            XData('CARD32', 1, dlen),
            XData('STRING8', dlen, data + '\0'))
...
class _NVCtrlQueryValidAttributeValuesRequest:
    ...
    def __init__(self, opcode, target_id, target_type, display_mask, attr):
        self.encoding = minx.encode(
            XData('CARD8', 1, opcode),
            XData('CARD8', 1, _X_nvCtrlQueryValidAttributeValues),
            XData('CARD16', 1, 4),
            XData('CARD16', 1, target_id),
            XData('CARD16', 1, target_type),
            XData('CARD32', 1, display_mask),
...
class _NVCtrlQueryValidAttributeValuesReply:
    ...
    def __init__(self, encoding: bytes) -> None:
        xreply, ad = minx.decode(encoding,
                                 ...
                                 XData('CARD32', 1, 'length'),
                                 XData('CARD32', 1, 'flags'),
                                 XData('INT32', 1, 'attr_type'),
                                 XData('INT32', 1, 'min'),
                                 XData('INT32', 1, 'max'),
                                 XData('CARD32', 1, 'bits'),
                                 XData('CARD32', 1, 'perms'))
        self.type = xreply['type']
        self.pad0 = xreply['pad0']
        self.sequence_number = xreply['sequence_number']
        self.length = xreply['length']
        self.flags = xreply['flags']
        self.attr_type = xreply['attr_type']
        self.min = xreply['min']
        self.max = xreply['max']
        self.bits = xreply['bits']
        self.perms = xreply['perms']
...
class _NVCtrlStringOperationRequest:
    def __init__(self, opcode, target_id, target_type, display_mask, attr, data):
        ...
        self.encoding = minx.encode(
            XData('CARD8', 1, opcode),
            XData('CARD8', 1, _X_nvCtrlStringOperation),
            XData('CARD16', 1, 5 + (((dlen + 3) & ~3) >> 2)),
            XData('CARD16', 1, target_id),
            XData('CARD16', 1, target_type),
            XData('CARD32', 1, display_mask),
...
class NVidiaControlLowLevel:
    def __init__(self) -> None:
        ...
        self.x_socket = None  # X connection socket
        self.x_screen = None  # X screen
        self.opcode = -1  # major opcode for X extension
        self.version = (-1, -1)  # NV-CONTROL extension version

        self.gpucount = 0  # number of GPUs in the system
    ...
    def open(self) -> None:
        ...
        while try_count < _MAX_TRY_COUNT:
            try:
                name, host, displayno, self.x_screen = xnet.get_x_display()
                self.x_socket, self.x_conn = minx.x_connect()
                try:
                    NVCtrl = minx.x_query_extension(self.x_socket, 'NV-CONTROL')
                    self.opcode = NVCtrl.major_opcode
                except Exception as e:
                    if self.x_socket is not None:
                        self.x_socket.close()
                    raise e
                ...
                LOG.exception("Try #%d failed.", try_count)
                try_count += 1
    ...
    def get_version(self) -> Tuple[int, int]:
        ...
        binrp = minx.x_change(self.x_socket, rq)
    ...
    def query_int_attribute(self, target: Target, displays: List, attr: int) -> NVCtrlQueryAttributeReply:
    ...
    def set_int_attribute(self, target: Target, displays: List, attr: int,
                          value: int) -> NVCtrlSetAttributeAndGetStatusRequest:
    ...
    def query_string_attribute(self, target: Target, displays: List, attr: int) -> NVCtrlQueryStringAttributeReply:
        ...
        # time.sleep(0.2)
        display_mask = self._displays2mask(displays)
        rq = _NVCtrlQueryStringAttributeRequest(self.opcode, target.id(), target.type(), display_mask, attr)
        ...
        while try_count < _MAX_TRY_COUNT:
            try:
                ...
                if binrp[0] == 0:
                    ...
                    if reply.flags == 0:
                        raise ValueError("Error while querying string attribute")
    ...
    def query_binary_data(self, target: Target, displays: List, attr: int):
        """return binary data"""
        display_mask = self._displays2mask(displays)
    ...
    def query_valid_attr_values(self, target: Target, displays: List, attr: int):
        display_mask = self._displays2mask(displays)
    ...
    def _displays2mask(self, displays: List) -> int:
